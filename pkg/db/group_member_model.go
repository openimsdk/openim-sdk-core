...
func (d *DataBase) GetGroupMemberListSplit(ctx context.Context, groupID string, filter int32, offset, count int) ([]*model_struct.LocalGroupMember, error) {
	...
	var err error
	if filter == 0 {
		err = d.conn.WithContext(ctx).Where("group_id = ? And role_level > ?", groupID, filter).Order("role_level ASC").Order("join_time ASC").Offset(offset).Limit(count).Find(&groupMemberList).Error
		...
		err = d.conn.WithContext(ctx).Where("group_id = ? And role_level > 1 ", groupID).Order("role_level ASC").Order("join_time ASC").Offset(offset).Limit(count).Find(&groupMemberList).Error
	} else {
