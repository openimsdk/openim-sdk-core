...
func (d *DataBase) ResetConversation(conversationID string) error {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	c := LocalConversation{ConversationID: conversationID, UnreadCount: 0, LatestMsg: "", LatestMsgSendTime: time.Time{}, DraftText: "", DraftTextTime: time.Time{}}
...
func (d *DataBase) ClearConversation(conversationID string) error {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	c := LocalConversation{ConversationID: conversationID, UnreadCount: 0, LatestMsg: "", DraftText: "", DraftTextTime: time.Time{}}
...
func (d *DataBase) RemoveConversationDraft(conversationID, draftText string) error {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	c := LocalConversation{ConversationID: conversationID, DraftText: draftText, DraftTextTime: time.Time{}}
