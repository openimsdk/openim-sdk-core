...
func (d *DataBase) SetConversationDraft(conversationID, draftText string) error {
	...
	t := d.conn.Debug().Exec("update conversation set draft_text=?,draft_text_time=?,latest_msg_send_time=case when latest_msg_send_time=? then ? else latest_msg_send_time  end where conversation_id=?",
...
func (d *DataBase) UnPinConversation(conversationID string, isPinned int) error {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	t := d.conn.Debug().Exec("update conversation set is_pinned=?,draft_text_time=case when draft_text=? then ? else draft_text_time  end where conversation_id=?",
