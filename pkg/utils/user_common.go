...
func (u *open_im_sdk.UserRelated) jsonUnmarshalAndArgsValidate(s string, args interface{}, callback commom.Base, operationID string) error {
...
func (u *open_im_sdk.UserRelated) jsonUnmarshal(s string, args interface{}, callback commom.Base, operationID string) error {
	err := json.Unmarshal([]byte(s), args)
	if err != nil {
		if callback != nil {
			log.NewError(operationID, "Unmarshal failed ", err.Error(), s)
