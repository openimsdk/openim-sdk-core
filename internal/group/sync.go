...
func (g *Group) SyncGroup(ctx context.Context, groupID string) error {
	...
	// todo
	if err := g.groupSyncer.Sync(ctx, util.AppendNotNil(ServerGroupToLocalGroup(resp.GroupInfos[0])), util.AppendNotNil(dbGroup), util.NoticeChange[*model_struct.LocalGroup](g.listener.OnGroupInfoChanged)); err != nil {
		return err
	}
